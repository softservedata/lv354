...
public abstract class AHeadPage {
    ...
    public double extractTotalCart() {
		return RegexUtils.extractFirstDouble(getCartButtonText ());			
	}
...
//ShoppingCartComponent
	 public PopupShoppingCartComponent createDropDownCart() {
		 PopupShoppingCartComponent result = null;
		 if (getCartButtonItemsCount()>0){
			 System.out.println("clickCartButton()");
			 clickCartButton();
			 return new PopupShoppingCartComponent (driver); 
		 }	 
	 return result;
}

	public HomePage emptyCart() {
		if (!Application.get().getCartStatusIsEmpty()) {
			PopupShoppingCartComponent sh = createDropDownCart();
			while (sh != null) {
			    sh.getPopupProductCartComponentsList().get(0).clickButton();
				sh = createDropDownCart();
			}
			Application.get().setCartStatusIsEmpty(true);
		}
		return new HomePage(driver);
	}
